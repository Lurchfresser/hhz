// src/main.rs in your temporary generator project

use std::fs::File;
use std::io::{self, Write};

/// This is the raw Polyglot Zobrist key array.
///
/// TODO: PASTE THE 781 64-BIT UNSIGNED INTEGERS FROM THE POLYGLOT SOURCE HERE.
/// You need to convert them from the C++ `U64(0x...)` format to the Rust `0x...,` format.
const RANDOM_64: [u64; 781] = [
...
fn main() -> io::Result<()> {
    let mut file = File::create("src/polyglot_zobrists.rs")?;

    writeln!(file, "// This file was generated by the zobrist_generator program.")?;
    writeln!(file, "// It contains Zobrist keys that are compatible with Polyglot opening books.")?;
    writeln!(file, "// Do not edit this file manually.\n")?;

    // --- 1. Piece Zobrist Keys ---
    // The Polyglot spec: offset = 64 * kind_of_piece + square
    // This means the keys for each piece are in a contiguous block of 64.
    let piece_configs = [
        ("BLACK_PAWN", 0),
        ("WHITE_PAWN", 1),
        ("BLACK_KNIGHT", 2),
    ...
    writeln!(file, "// --- Piece Keys ---")?;
    for (name, kind_index) in piece_configs {
        // The spec uses `64 * kind + 8*row + file`, which simplifies to `64 * kind + square_index`
        // where square_index is 0 for a1, 1 for b1, ..., 63 for h8.
        let start_offset = kind_index * 64;
        let end_offset = start_offset + 64;
        let piece_keys = &RANDOM_64[start_offset..end_offset];

        write_array(&mut file, &format!("ZOBRISTS_{}S", name), piece_keys, None)?;
    }
    println!("Successfully generated Zobrist keys for all 12 piece types.");

    // --- 2. Castling Rights Keys ---
    // These start at offset 768 and there are 4 of them.
    let castle_offset = 768;
    let castle_keys = &RANDOM_64[castle_offset .. castle_offset + 4];
    let castle_comments = [
        "White kingside (K)",
        "White queenside (Q)",
        "Black kingside (k)",
    ...
    write_array(&mut file, "ZOBRISTS_CASTLING_RIGHTS", castle_keys, Some(&castle_comments))?;
    println!("Successfully generated Zobrist keys for castling rights.");


    // --- 3. En Passant File Keys ---
    // These start at offset 772 and there are 8 of them (one for each file).
    let ep_offset = 772;
    let ep_keys = &RANDOM_64[ep_offset .. ep_offset + 8];
    let ep_comments = [
    ...
    write_array(&mut file, "ZOBRISTS_EN_PASSANT_FILE", ep_keys, Some(&ep_comments))?;
    println!("Successfully generated Zobrist keys for en passant files.");


    // --- 4. Side to Move Key ---
    // This is the single key at offset 780, used when it's White's turn.
    let turn_offset = 780;
    let turn_key = RANDOM_64[turn_offset];

    writeln!(file, "\n// --- Side to Move Key ---")?;
    writeln!(file, "// This key is XORed into the hash when it is White's turn to move.")?;
    writeln!(file, "pub const ZOBRISTS_WHITE_TO_MOVE: u64 = 0x{:016x};", turn_key)?;
    println!("Successfully generated Zobrist key for side to move.");


    println!("\nAll done! Zobrist constants written to src/polyglot_zobrists.rs");
    Ok(())
}

/// A helper function to write a slice of u64 values as a Rust const array.
fn write_array(
    file: &mut File,
    name: &str,
    values: &[u64],
    ...
    writeln!(file, "pub const {}: [u64; {}] = [", name, values.len())?;
    for (i, &val) in values.iter().enumerate() {
        let comma = if i < values.len() - 1 { "," } else { "" };
        if let Some(comment_slice) = comments {
            if i < comment_slice.len() {
                writeln!(file, "    0x{:016x}{}\t// {}", val, comma, comment_slice[i])?;
                continue;
            }
        }
        writeln!(file, "    0x{:016x}{}", val, comma)?;
    }
    writeln!(file, "];")?;
    Ok(())
}